...
export function inferTierName(text) {
  const s = String(text || '').toLowerCase();
  if (!s) return null;
  if (s.indexOf('extreme') >= 0 || s.indexOf('unlimited') >= 0 || s === 'pro' || s === 'full') return 'extreme';
  if (s.indexOf('premium') >= 0 || s.indexOf('plus') >= 0) return 'premium';
  return null;
}
...
export function tierForPrice(price) {
  ...
  if (typeof price === 'string') {
    if (price === PRICE_TO_TIER.extreme) return 'extreme';
    if (price === PRICE_TO_TIER.premium) return 'premium';
    return inferTierName(price);
  }
  const id = price.id || '';
  if (id === PRICE_TO_TIER.extreme) return 'extreme';
  if (id === PRICE_TO_TIER.premium) return 'premium';
  return inferTierName(price.nickname) || inferTierName(price.lookup_key) || inferTierName(price.product);
}
...
export async function activeTierForCustomer(customer) {
  const customerId = customer && (customer.id || customer);
  const metaTier = inferTierName(customer && customer.metadata && customer.metadata.gd_tier);
  if (!customerId) return { tier: metaTier || 'free', customerId: null, subscriptionId: null };
  ...
  let tier = metaTier || 'free';
  let subscriptionId = null;
  subs.forEach(function (s) {
    const price = s.items && s.items.data && s.items.data[0] && s.items.data[0].price;
    const t = tierForPrice(price) || inferTierName(s.metadata && s.metadata.gd_plan);
