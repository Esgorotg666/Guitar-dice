...
import { scaleRunNotes, randomScalePattern, moveItem } from './scaleRun';

export { moveItem, scaleRunNotes, randomScalePattern };
...
export function assembleRoll(opts) {
  ...
  slots.forEach(function (face, i) {
    ...
    if (face === 'solo') {
      ...
      if (item.scale) {
        const phrase = randomScalePattern(item.scale.root, item.scale.mode, { bars: 4 });
        item.run = phrase.notes.length ? phrase.notes : scaleRunNotes(item.scale.root, item.scale.mode);
        item.runDevices = phrase.devices || [];
      } else {
        item.run = [];
      }
    }
