...
export const CLARITY = {
  lead: { k: 0.88, min: 0.72 },
  chord: { k: 0.80, min: 0.58 },
  tuner: { k: 0.93, min: 0.90 },
  default: { k: 0.86, min: 0.65 }
};
...
function keyMaxima(nsdf, minTau) {
  ...
  for (i = Math.max(2, minTau); i < nsdf.length - 1; i++) {
    if (nsdf[i] > nsdf[i - 1] && nsdf[i] >= nsdf[i + 1] && nsdf[i] > 0.45) {
      peaks.push(i);
    }
  }
  return peaks;
...
export function detectPitchInfo(buf, sampleRate, mode) {
  ...
  if (rmsOf(buf) < 0.006) return empty;
...
export function matchesPitch(freq, target, windowCents) {
  return Math.abs(centsOffOctave(freq, target)) <= (windowCents || 90);
}
