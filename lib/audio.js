...
export async function playSequence(notes, bpm, onStep, onEnd) {
  ...
  notes.forEach(function (nt, i) {
    ...
    const hold = vib.vib ? Math.max(1.6, spb * beats * 2.6) : Math.max(0.9, spb * beats * (legato ? 1.8 : 2.3));
    ...
    if (nt.pick === 'T') {
      ...
      pluckOn(c, freq, t, Math.min(dur, muted ? 0.55 : 3.4), vol, nt.string, gate, opts);
    }
