...
export function facesForTier(tier, allowBridge) {
  ...
  if (t.indexOf('extreme') >= 0 || t.indexOf('premium') >= 0) {
    return FACE_ORDER.filter(function (id) { return id !== 'bridge' || allowBridge || t.indexOf('extreme') >= 0; });
  }
  return ['chord', 'scale', 'strum', 'rhythm'];
}
