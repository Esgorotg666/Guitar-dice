...
import { lessonsFor, isStretch } from '../lib/lessons';
...
export default function Home() {
  ...
  if (loading) {
    return <Shell genre={genre}><div className="center"><div className="spinner" /><p className="muted">Loading Guitar Dice...</p></div></Shell>;
  }

  if (needsOnboarding) {
    return (
      <Shell genre={draft.preferredGenre || 'rock'}>
  ...
  const lessons = lessonsFor(allLessons, style, genre, skill);
  ...
  return (
    <Shell genre={genre}>
      ...
      <nav className="tabs">
        {[['roll','Roll'],['song','Song'],['classroom','Class'],['tools','Tools'],['saved','Saved'],['settings','Style'],['plans','Plans'],['account','Account']].map(function (t) {
            ...
            <div>
              <div className="card">
                ...
                <p className="muted sm">
                  {lessons.length} lessons for {styleMeta.label.toLowerCase()}, sorted for {genreLabel} at {skill} level. {genreLabel} lessons come first, then universal technique.
                </p>
              </div>
      ...
      <footer className="appFoot"><a href="/privacy">Privacy</a></footer>
    </Shell>
  );
}
...
function Shell(props) {
  return (
    <div className="themeRoot" data-genre={props.genre || 'rock'}>
      <Head>
