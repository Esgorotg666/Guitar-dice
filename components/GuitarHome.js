...
import DiceFocus from './DiceFocus';
import SongBuilder from './SongBuilder';
import ClassroomTab from './ClassroomTab';
import SettingsTab from './SettingsTab';
import FreeAdSlot from './FreeAdSlot';
import AvatarMark from './AvatarMark';
import LockerStudio from './LockerStudio';
import RewardedAd from './RewardedAd';
import { genreLook } from '../lib/genreLook';
import { STYLES, GENRES, SKILLS, practiceTip } from '../lib/style';
...
import { loadLocker, saveLocker, guitarTitle } from '../lib/locker';
import { NECK_SYSTEM_LESSONS } from '../lib/neckSystems';
import { defaultSlots, sanitiseSlots } from '../lib/bridge';
import { assembleRoll, leadScaleFor, moveItem } from '../lib/rollArrange';
import { strumChord, playProgressionChords, playSequence } from '../lib/audio';
import { chordColor, chordPitchClasses, voicingLabel, chordSheet, scaleForChord } from '../lib/theory';
...
export default function Home() {
  ...
  const [locker, setLocker] = useState({ avatar:{ skin:'tan', hair:'none', hat:'none', shirt:'black' }, guitar:{ body:'natural', guard:'none', hardware:'chrome', inlay:'dots' } });
  ...
  function setCount(n) {
    ...
    setSlots(function (prev) {
      ...
      while (next.length < n) next.push('verse');
  ...
  function doRoll() {
    ...
    spendRoll(function (ok, body) {
      ...
      const useSlots = canBridge ? sanitiseSlots(slots.slice(0, allowed)) : defaultSlots(allowed, style);
      setTimeout(function () {
        const out = assembleRoll({
          slots: useSlots,
          style: style,
          genre: genre,
          skill: skill,
        ...
        setScale(leadScaleFor(out, { style:style, genre:genre, skill:skill, modes:data.modes, lockRoot:rollKey, lockMode:rollMode }));
        setItems(out);
        setRecent(out.map(function (c) { return c.key; }).concat(recent).slice(0, 24));
  ...
  function exportSheet() {
    const text = chordSheet(items, {
      ...
      scale: scale && scale.mode ? scale.root + ' ' + scale.mode.name : null
    });
    if (navigator.share) navigator.share({ title:'Guitar Dice progression', text:text }).catch(function () {});
    else if (navigator.clipboard) { navigator.clipboard.writeText(text); setMsg('Chord sheet copied.'); }
  }
  ...
  async function tapChord(i) {
    ...
    const it = items[i];
    if (!it || !it.chord) return;
    if (it.kind === 'solo' && it.run && it.run.length) {
      const ok = await playSequence(it.run, 100);
      setAudioWarn(!ok);
      return;
    }
    const ok = await strumChord(it.chord.positions, true);
    setAudioWarn(!ok);
  }
  function moveRoll(i, dir) {
    setItems(moveItem(items, i, dir));
  }
        ...
        <div className="onboard">
          ...
          {step === 1 ? (
            <div>
              ...
              <p className="muted">What do you play most? This picks your rolls and your lesson list.</p>
          ...
          {step === 2 ? (
            <div>
              <h1>Genre</h1>
              <p className="muted">This weights the chords. It does not change how the app talks.</p>
              <div className="grid">
          ...
          {step === 3 ? (
            <div>
              <h1>Skill</h1>
              <p className="muted">Entry Level includes Day 1 setup. Other levels skip hold, strings, and tune.</p>
              <div className="choices">
  ...
  const selScale = sel ? (sel.scale || (isBridgeSel ? sel.link : (data ? scaleForChord(sel.key, data.modes) : null))) : null;
  ...
  return (
    <Shell genre={genre} theme={resolvedTheme(appSettings.theme)}>
      ...
      <header className="appHead">
        <button className="idChip" onClick={function () { setTab('account'); }} title="Open locker">
          <AvatarMark locker={locker} size={40} />
        </button>
        <div>
          <h1>Guitar Dice</h1>
          <p className="muted sm">{styleMeta.label} · {genreLabel}</p>
        </div>
        <div className="stats">
          {paid ? <span className="pill tier">{usage.tierLabel}</span> : null}
          {streak && streak.currentStreak > 0 ? <span className="pill">{streak.currentStreak} day{streak.currentStreak === 1 ? '' : 's'}</span> : null}
          <span className="pill">{usage && usage.unlimitedRolls ? 'unlimited' : ((usage && usage.remainingRolls) || 0) + ' rolls'}</span>
        </div>
      </header>

      <nav className="tabs">
        {[['roll','Roll'],['classroom','Class'],['tools','Tools'],['song','Song'],['saved','Saved'],['settings','Settings'],['account','You']].map(function (t) {
      ...
      {tab === 'roll' ? (
        <div>
          <p className="homeIntro">Tap <b>Roll chords</b>. Then tap a chord to hear it. Class is the lessons.</p>
          <button className={'rollBtn' + (rolling ? ' rolling' : '')} onClick={doRoll} disabled={rolling}>
            <span className="die">[ ]</span>
            {rolling ? 'Rolling…' : (items.length ? 'Roll again' : 'Roll chords')}
          </button>
          {msg ? <div className="notice"><span>{msg}</span></div> : null}
          <details className="moreRoll">
            <summary>Key and mode (optional)</summary>
          ...
          {diceMax > 2 ? (
            <DiceConfig max={diceMax} count={diceN} slots={slots} allowBridge={canBridge} tier={tier}
            ...
            <div className="card">
              ...
              <div className="chordRow">
                {items.map(function (it, i) {
                  ...
                  return (
                    <div key={(it.key || 'x') + '-' + i}
                      className={'chordCard' + (it.kind === 'solo' ? ' bridgeCard' : '') + (selected === i ? ' sel' : '')}
                      style={{ borderColor: selected === i ? col.dot : '#1e2a35' }}
                      onClick={function () { tapChord(i); }}>
                      {it.label ? <span className="bridgeTag">{it.label}</span> : null}
                      <strong>
                        <span className="chordIdx" style={{ background:col.dot, color:col.text }}>{i+1}</span>
                        {it.chord && it.chord.name}
                      </strong>
                      <span className="chordPos">{voicingLabel(it.chord)}</span>
                      <ChordDiagram chord={it.chord} accent={col} />
                      <div className="optRow" style={{ marginTop:8 }} onClick={function (e) { e.stopPropagation(); }}>
                        <button type="button" className="btn ghost sm" disabled={i===0} onClick={function () { moveRoll(i, -1); }}>Left</button>
                        <button type="button" className="btn ghost sm" disabled={i===items.length-1} onClick={function () { moveRoll(i, 1); }}>Right</button>
                      </div>
                    </div>
                  );
                })}
              ...
              <p className="muted sm" style={{ marginTop:8 }}>
                {audioWarn ? 'No sound? On iPhone the side silent switch mutes web audio.' : 'Tap a chord to hear it. Solo dice play the scale. Use Left / Right to rearrange the loop.'}
              </p>
            </div>
          ) : null}
          {sel && selScale && selScale.mode ? (
            <div className="card">
              <div className="rowBetween">
                <h3>{selScale.root} {selScale.mode.name}</h3>
                <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                  <span className="tagBlue">{sel.kind === 'solo' ? 'Solo scale' : 'over ' + (sel.chord && sel.chord.name)}</span>
              ...
              <Fretboard guitar={locker.guitar} root={selScale.root} mode={selScale.mode} showNotes={showNotes}
                overlays={[{ color:selColor, pcs:chordPitchClasses(sel.chord && sel.chord.positions) }]} />
              <div className="legend">
                <span className="legendItem"><span className="swatch" style={{ background:'#e8eef5' }} />root ({selScale.root})</span>
                <span className="legendItem"><span className="swatch" style={{ background:selColor.dot }} />notes in {sel.chord && sel.chord.name}</span>
              </div>
          ...
          {items.length && canLoop ? <LoopPlayer chords={items.map(function (i) { return i.chord; })} labels={items.map(function (i) { return i.label; })} onMove={moveRoll} /> : null}
          {items.length && !canLoop ? (
            <div className="card">
              <h3>Loop player</h3>
              <p className="muted sm">Loop this progression as a backing track. Available on Premium and Extreme.</p>
          ...
          {!items.length ? <div className="empty"><p className="muted">Nothing rolled yet.</p></div> : null}
        </div>
      ) : null}

      {tab === 'account' ? (
        <div>
          <LockerStudio locker={locker} progress={pathProgress} name={(user && user.username) || 'Player'}
            onChange={function (next) { setLocker(saveLocker(next, pathProgress)); }} />
          <Account user={user} onChange={refresh} />
        </div>
      ) : null}
      ...
      {tab === 'classroom' ? (
        <ClassroomTab
          ...
          setProgress={function (next) {
            setPathProgress(next);
            setLocker(saveLocker(locker, next));
          }}
      ...
      {tab === 'saved' ? (
        <div className="card">
          ...
          {!saved.length ? <p className="muted">Nothing saved yet. Roll something you like and tap Save.</p> : null}
      ...
      {tab === 'settings' ? (
        <SettingsTab settings={appSettings} user={user} style={style} onChange={function (next) { setAppSettings(saveSettings(next)); }}>
        <LockerStudio locker={locker} progress={pathProgress} name={(user && user.username) || 'Player'}
          onChange={function (next) { setLocker(saveLocker(next, pathProgress)); }} />
        <div className="card">
          <h3>Playing Style</h3>
          <p className="muted sm">Changes voicings and the lesson list.</p>
          <div className="choices">
            {STYLES.map(function (s) {
              return (
          ...
          <p className="muted sm">Weights the chords you roll.</p>
          <div className="grid">
            {GENRES.map(function (g) {
              return (
          ...
          <p className="muted sm">Entry Level includes Day 1 setup. Other levels skip it.</p>
          <div className="choices">
            {SKILLS.map(function (s) {
              return (
