...
function isDay1Id(id) {
  return String(id || '').indexOf('day1-') === 0;
}
...
export function unlockedIndex(progress, style, skill) {
  const steps = pathFor(style, skill);
  let i = 0;
  while (i < steps.length && isCleared(progress, steps[i].id)) i++;
  return i;
}

export function isUnlocked(progress, style, lessonId, gate, fullAccess) {
  ...
  var i;
  for (i = idx - 1; i >= 0; i--) {
    if (isDay1Id(steps[i].id) && !isDay1Id(lessonId)) continue;
    return isCleared(progress, steps[i].id);
  }
  return true;
}
...
function dayOneComplete(progress, style) {
  const steps = pathFor(style, 'entry').filter(function (s) { return isDay1Id(s.id); });
  return steps.length > 0 && steps.every(function (s) { return isCleared(progress, s.id); });
}
...
export function recordAttempt(progress, lessonId, style, score) {
  ...
  if (dayOneComplete(next, style)) next.badges['day-one'] = true;
