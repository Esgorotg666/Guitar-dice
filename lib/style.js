...
function fromLibrary(opts) {
  ...
  const fresh = shuffle(keys.filter(function (k) { return avoid.indexOf(k) === -1; }));
  const bag = fresh.concat(shuffle(keys));
  const want = ['major', 'minor', 'seventh', 'sus', 'color'];
  const out = [];
  const usedFam = { major:0, minor:0, seventh:0, sus:0, color:0 };
  function pushKey(key) {
    if (!chords[key]) return false;
    if (out.some(function (c) { return c.key === key; })) return false;
  ...
  want.forEach(function (fam) {
    if (out.length >= (opts.count || 4)) return;
    let i;
    for (i = 0; i < bag.length; i++) {
      if (familyOf(parseSuffix(bag[i])) === fam && pushKey(bag[i])) break;
    }
  });
  let i;
  for (i = 0; i < bag.length && out.length < (opts.count || 4); i++) pushKey(bag[i]);
  return out;
}
...
export function rollProgression(opts) {
  ...
  const mixAll = opts.mixAll !== false;
