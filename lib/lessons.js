...
import { MASTER_PACK_1 } from './masterPack1';
import { MASTER_PACK_2 } from './masterPack2';
import { ADVANCED_PACK } from './advancedPack';
import { FLAMENCO_LESSONS } from './flamencoPalos';
...
function isStarter(lesson) {
  return lesson && (lesson.track === 'day1' || lesson.track === 'barre' || lesson.track === 'sheet' || lesson.track === 'path' || lesson.track === 'study' || lesson.track === 'flamenco' || lesson.capstone);
}
...
export function isThinLesson(lesson) {
  ...
  if (String(lesson.id || '').indexOf('bank-') === 0) return true;
...
export function enrichLesson(lesson) {
  ...
  var notes = (lesson.track === 'sheet' || lesson.track === 'path' || lesson.track === 'study' || lesson.track === 'flamenco' || grouped)
...
export function mergeLessonLists(base, extra) {
  var map = {};
  var order = [];
  (base || []).concat(extra || []).concat(DAY_ONE_LESSONS).concat(BARRE_LESSONS).concat(PATH_CURRICULUM).concat(SHEET_LESSONS).concat(STUDY_LESSONS).concat(MASTER_PACK_1).concat(MASTER_PACK_2).concat(ADVANCED_PACK).concat(CRAFTED_LESSONS).concat(SKILL_LESSONS).concat(VIB_LESSONS).concat(EXPERT_LESSONS).concat(HAND_DRILLS).concat(CIRCLE_LESSONS).concat(HARMONY_LESSONS).concat(DOMINANT_LESSONS).concat(CAPSTONE_LESSONS).concat(FLAMENCO_LESSONS).forEach(function (l) {
    if (!l || !l.id) return;
    if (String(l.id).indexOf('bank-') === 0) return;
...
export function lessonsFor(all, style, genre, skill) {
  ...
  function rank(l) {
    ...
    const path = id.indexOf('path-') === 0 || id.indexOf('sheet-') === 0 || id.indexOf('study-') === 0 || id.indexOf('master-') === 0 || id.indexOf('adv-') === 0 || id.indexOf('m2-') === 0 || id.indexOf('fla-') === 0 ? 0 : 1;
    const neck = id.indexOf('neck-') === 0 ? 0 : 1;
    const crafted = isGrouped(l.notes) ? 0 : 1;
    const flamenco = l.genre === 'flamenco' && genre === 'flamenco' ? 0 : (l.genre === 'flamenco' ? 2 : 1);
    return {
