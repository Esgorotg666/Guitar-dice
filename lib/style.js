...
export function rollProgression(opts) {
  ...
  while (out.length < count && guard < 700) {
    ...
    if (diatonic) {
      ...
      if (opts.skill !== 'entry' && suffix === '' && Math.random() < 0.35 && suffixes.indexOf('maj7') !== -1) suffix = 'maj7';
      if (opts.skill !== 'entry' && suffix === 'm' && Math.random() < 0.35 && suffixes.indexOf('m7') !== -1) suffix = 'm7';
      if (opts.skill !== 'entry' && suffix === '' && lockedMode === 'mixolydian' && Math.random() < 0.4 && suffixes.indexOf('7') !== -1) suffix = '7';
    } else {
      root = pick(ALL_ROOTS);
