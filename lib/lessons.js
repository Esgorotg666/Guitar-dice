...
export function enrichLesson(lesson) {
  ...
  if (lesson.track === 'day1') extra = '';
  if (extra && summary.indexOf('10–12 minute block') === -1 && summary.indexOf('full chord shape') === -1) summary += extra;
  var watch = lesson.watchFor || '';
  if (watch && watch.indexOf('drop the tempo') === -1 && lesson.track !== 'day1') {
    watch = watch.replace(/\.+$/, '') + '. If a note dies or a string squeals, drop the tempo before you add speed.';
  }
  var seen = {};
  var steps = [];
  (lesson.steps || []).concat((grouped || lesson.track === 'day1') ? [] : extraSteps(lesson)).forEach(function (s) {
    var k = String(s || '').trim().toLowerCase();
    if (!k || seen[k]) return;
    seen[k] = true;
    steps.push(s);
  });
  var notes = (grouped || change || lesson.track === 'day1') ? fillBeats(lesson.notes || [], style) : expandNotes(lesson.notes || [], style);
...
export function mergeLessonLists(base, extra) {
  var map = {};
  var order = [];
  (base || []).concat(extra || []).concat(DAY_ONE_LESSONS).concat(CRAFTED_LESSONS).concat(SKILL_LESSONS).concat(VIB_LESSONS).concat(EXPERT_LESSONS).concat(HAND_DRILLS).concat(CIRCLE_LESSONS).concat(HARMONY_LESSONS).concat(DOMINANT_LESSONS).forEach(function (l) {
    if (!l || !l.id) return;
...
export function lessonsFor(all, style, genre, skill) {
  ...
  function rank(l) {
    ...
    const day1 = String(l.id || '').indexOf('day1-') === 0 ? 0 : 1;
