...
function isStarter(lesson) {
  return lesson && (lesson.track === 'day1' || lesson.track === 'barre' || lesson.capstone);
}
...
export function enrichLesson(lesson) {
  ...
  var notes = (grouped || change || isStarter(lesson)) ? fillBeats(lesson.notes || [], style) : expandNotes(lesson.notes || [], style);
...
export function mergeLessonLists(base, extra) {
  var map = {};
  var order = [];
  (base || []).concat(extra || []).concat(DAY_ONE_LESSONS).concat(BARRE_LESSONS).concat(CRAFTED_LESSONS).concat(SKILL_LESSONS).concat(VIB_LESSONS).concat(EXPERT_LESSONS).concat(HAND_DRILLS).concat(CIRCLE_LESSONS).concat(HARMONY_LESSONS).concat(DOMINANT_LESSONS).concat(CAPSTONE_LESSONS).forEach(function (l) {
    if (!l || !l.id) return;
