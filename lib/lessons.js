...
export function mergeLessonLists(base, extra) {
  var map = {};
  var order = [];
  (base || []).concat(extra || []).concat(DAY_ONE_LESSONS).concat(BARRE_LESSONS).concat(PATH_CURRICULUM).concat(SHEET_LESSONS).concat(STUDY_LESSONS).concat(MASTER_PACK_1).concat(ADVANCED_PACK).concat(CRAFTED_LESSONS).concat(SKILL_LESSONS).concat(VIB_LESSONS).concat(EXPERT_LESSONS).concat(HAND_DRILLS).concat(CIRCLE_LESSONS).concat(HARMONY_LESSONS).concat(DOMINANT_LESSONS).concat(CAPSTONE_LESSONS).forEach(function (l) {
    if (!l || !l.id) return;
...
export function lessonsFor(all, style, genre, skill) {
  ...
  function rank(l) {
    ...
    const path = id.indexOf('path-') === 0 || id.indexOf('sheet-') === 0 || id.indexOf('study-') === 0 || id.indexOf('master-') === 0 || id.indexOf('adv-') === 0 ? 0 : 1;
