...
function isChordChangeLesson(lesson) {
  return lesson && lesson.style !== 'lead' && Array.isArray(lesson.chords) && lesson.chords.length >= 2;
}
...
export function enrichLesson(lesson) {
  ...
  if (summary.indexOf('10–12 minute block') === -1 && summary.indexOf('full chord shape') === -1) summary += extra;
  ...
  var notes = change ? fillBeats(lesson.notes || [], style) : expandNotes(lesson.notes || [], style);
  if (change && notes[0]) notes[0] = Object.assign({}, notes[0], { chordKeys: lesson.chords.slice() });
...
export function mergeLessonLists(base, extra) {
  var seen = {};
  var out = [];
  (base || []).concat(extra || []).forEach(function (l) {
