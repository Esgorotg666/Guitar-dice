...
import { ownsLayout } from './shopLayouts';
import { hasTier as planHas } from './entitlements';

const KEY = 'gd-locker-v1';
const PREMIUM_BOARDS = { blank: 1, birds: 1, split: 1 };
const EXTREME_BOARDS = { blank: 1, birds: 1, split: 1, shark: 1, glow: 1 };
...
export const LAYOUTS = [
  ...
  { id: 'blank', label: 'Blank board', shop: 'sku', price: '$1.99', blurb: 'No inlays. Read the neck by notes only.' },
  { id: 'birds', label: 'Bird markers', shop: 'sku', price: '$2.99', blurb: 'Teardrop birds on 3-5-7-9-12.' },
  { id: 'split', label: 'Split blocks', shop: 'sku', price: '$2.99', blurb: 'Offset rectangles, two per 12.' },
  { id: 'shark', label: 'Shark fins', shop: 'sku', price: '$3.99', blurb: 'Triangles pointing toward the nut.' },
  { id: 'glow', label: 'Glow dots', shop: 'sku', price: '$3.99', blurb: 'Bright side-dot glow on every marker.' }
];
...
export function hasTier(tier, min) {
  return planHas(tier, min);
}

function layoutIncluded(id, tier) {
  const t = cachedTier(tier);
  if (planHas(t, 'extreme') && EXTREME_BOARDS[id]) return true;
  if (planHas(t, 'premium') && PREMIUM_BOARDS[id]) return true;
  return ownsLayout(id);
}
...
export function lockHint(opt, progress, tier) {
  if (!opt) return '';
  if (opt.shop === 'sku') {
    if (layoutIncluded(opt.id, tier)) return '';
    return 'Included on Extreme, or buy ' + (opt.price || 'a la carte');
  }
...
export function sanitize(raw, progress, tier) {
  ...
  function pick(list, kind, val, fallback) {
    ...
    if (hit && isOpen(kind, hit.id, progress, tier)) return hit.id;
    return fallback;
  }
