...
const OWN_KEY = 'gd-owned-layouts-v1';
...
export function cacheOwnedLayouts(ids) {
  const list = Array.isArray(ids) ? ids : [];
  if (typeof window !== 'undefined') {
    try { window.localStorage.setItem(OWN_KEY, JSON.stringify(list)); } catch (e) {}
  }
  return list;
}

export function cachedOwnedLayouts() {
...
export function ownsLayout(id) {
  return cachedOwnedLayouts().indexOf(id) >= 0;
}
