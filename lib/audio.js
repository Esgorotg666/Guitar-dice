...
let ctx = null, unlocked = false, bus = null, lastStop = null;
...
function buildBus(c) {
  ...
  const reverb = c.createConvolver(); reverb.buffer = buildImpulse(c, 1.2, 2.8);
  const master = c.createDynamicsCompressor();
  master.threshold.value = -12; master.knee.value = 18; master.ratio.value = 1.6;
  master.attack.value = 0.003; master.release.value = 0.12;
...
function resetMaster(c) {
  if (!bus || !c) return;
  try {
    const now = c.currentTime;
    bus.out.gain.cancelScheduledValues(now);
    bus.out.gain.setValueAtTime(1, now);
    bus.input.gain.cancelScheduledValues(now);
    bus.input.gain.setValueAtTime(1, now);
  } catch (e) {}
}
...
export function vibFromTech(tech) {
  const list = tech || [];
  const marked = hasTech(list, '~') || hasTech(list, 'VW') || hasTech(list, 'VF') || hasTech(list, 'VD') || hasTech(list, 'VC');
  if (!marked) return { vib:false };
  if (hasTech(list, 'VC')) return { vib:true, vibRate:5.2, vibDepth:0.0075, vibDelay:0.1, vibAround:true };
  if (hasTech(list, 'VW')) return { vib:true, vibRate:2.3, vibDepth:0.03, vibDelay:0.2, vibAround:false };
  if (hasTech(list, 'VF')) return { vib:true, vibRate:6.4, vibDepth:0.009, vibDelay:0.07, vibAround:false };
  return { vib:true, vibRate:4.1, vibDepth:0.014, vibDelay:0.22, vibAround:false };
}

function applyVibrato(src, time, dur, opts) {
  if (!opts || !opts.vib) return;
  const base = opts.bendSemis ? Math.pow(2, opts.bendSemis / 12) : 1;
  const delay = opts.bendSemis ? Math.max(opts.vibDelay || 0.12, Math.min(0.32, dur * 0.38)) : (opts.vibDelay || 0.14);
  const start = time + delay;
  if (start >= time + dur - 0.06) return;
  const hz = opts.vibRate || 4.2;
  const depth = opts.vibDepth || 0.014;
  const around = !!opts.vibAround;
  let t = start;
  let up = true;
  src.playbackRate.setValueAtTime(base, start);
  while (t < time + dur - 0.05) {
    const half = 0.5 / hz;
    const target = around ? (up ? base * (1 + depth) : base * (1 - depth)) : (up ? base * (1 + depth) : base);
    src.playbackRate.linearRampToValueAtTime(Math.max(0.5, target), t + half);
    t += half;
    up = !up;
  }
}
...
function pluckOn(c, freq, time, dur, gainVal, stringIdx, dest, opts) {
  ...
  const len = Math.ceil(sr * Math.min(dur, opts.muted ? 1.1 : 2.4));
  ...
  applyVibrato(src, time, dur, opts);
...
export async function playSequence(notes, bpm, onStep, onEnd) {
  if (lastStop) { try { lastStop(); } catch (e) {} lastStop = null; }
  const c = await ensureAudio();
  ...
  notes.forEach(function (nt, i) {
    ...
    const vib = vibFromTech(tech);
    const freq = noteFreq(nt.string, nt.fret);
    const slideFrom = slide ? noteFreq(nt.string, Math.max(0, (nt.fret || 0) - 2)) : 0;
    const hold = vib.vib ? Math.max(1.2, spb * beats * 1.8) : Math.max(0.7, spb * beats * (legato ? 1.4 : 1.6));
    const dur = muted ? Math.max(0.18, spb * beats * 0.85) : hold;
    const vol = muted ? 0.4 : (legato ? 0.46 : 0.5);
    const opts = Object.assign({ muted:muted, legato:legato, slideFrom:slideFrom, bendSemis: bend ? 2 : 0 }, vib);
    if (nt.pick === 'T') {
      ...
      pluckOn(c, freq, t, Math.min(dur, muted ? 0.55 : 2.2), vol, nt.string, gate, opts);
    }
  ...
  function stop() {
    ...
    try {
      ...
      gate.gain.linearRampToValueAtTime(0.0001, now + 0.04);
      setTimeout(function () {
        try { gate.disconnect(); } catch (e) {}
        resetMaster(c);
      }, 80);
    } catch (e) { resetMaster(c); }
    if (onStep) onStep(-1);
    if (lastStop === stop) lastStop = null;
  }
  lastStop = stop;
  return stop;
}
...
export async function strumChord(positions, downstroke) {
  const c = await ensureAudio();
  if (!c) return false;
  pluckOn(c, noteFreq(0, 0), c.currentTime, 0.01, 0.0001, 0);
  ...
  order.forEach(function (nt, i) {
    pluckOn(c, noteFreq(nt.s, nt.f), t0 + i*spread, 2.2, 0.4 * (downstroke === false ? 0.86 : 1) * (1 - i*0.014), nt.s);
  });
  return true;
}
...
export async function playProgressionChords(chords, gapMs) {
  ...
  chords.forEach(function (ch, ci) {
    ...
    seq.forEach(function (nt, i) {
      pluckOn(c, noteFreq(nt.s, nt.f), t + i*0.018, Math.max(1.2, gap*1.4), 0.4, nt.s);
    });
    t += gap;
  });
  return true;
...
export async function startLoop(chords, bpm, barsPerChord, withClick, onBar, clickSound, beatsPerBar) {
  ...
  function scheduleOne() {
    ...
    seq.forEach(function (nt, i) {
      pluckOn(c, noteFreq(nt.s, nt.f), nextTime + i*0.018, Math.min(barLen*1.1, 2.2), 0.36, nt.s, gate);
    });
    if (withClick) {
  ...
  function stop() {
    ...
    try {
      ...
      gate.gain.cancelScheduledValues(now);
      gate.gain.setValueAtTime(1, now);
      gate.gain.linearRampToValueAtTime(0.0001, now + 0.05);
      setTimeout(function () {
        try { gate.disconnect(); } catch (e) {}
        resetMaster(c);
      }, 90);
