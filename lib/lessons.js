...
function isStarter(lesson) {
  return lesson && (lesson.track === 'day1' || lesson.track === 'barre' || lesson.track === 'sheet' || lesson.track === 'path' || lesson.capstone);
}
...
export function enrichLesson(lesson) {
  ...
  var notes = (lesson.track === 'sheet' || lesson.track === 'path' || grouped)
...
export function mergeLessonLists(base, extra) {
  var map = {};
  var order = [];
  (base || []).concat(extra || []).concat(DAY_ONE_LESSONS).concat(BARRE_LESSONS).concat(PATH_CURRICULUM).concat(SHEET_LESSONS).concat(CRAFTED_LESSONS).concat(SKILL_LESSONS).concat(VIB_LESSONS).concat(EXPERT_LESSONS).concat(HAND_DRILLS).concat(CIRCLE_LESSONS).concat(HARMONY_LESSONS).concat(DOMINANT_LESSONS).concat(CAPSTONE_LESSONS).forEach(function (l) {
    if (!l || !l.id) return;
...
export function lessonsFor(all, style, genre, skill) {
  ...
  function rank(l) {
    ...
    const path = id.indexOf('path-') === 0 || id.indexOf('sheet-') === 0 ? 0 : 1;
