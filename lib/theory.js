...
export function tuningStrings(id, a4) {
  const ref = Number(a4) > 0 ? Number(a4) : 440;
  const scale = ref / 440;
  ...
  return t.offsets.map(function (off, i) {
    return {
      ...
      freq: Math.round(STD_FREQ[i] * scale * Math.pow(2, off / 12) * 100) / 100,
      string: 6 - i,
      idx: i,
      offset: off
    };
...
export function analysePitch(freq, a4) {
