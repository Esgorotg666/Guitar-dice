...
export function defaults() {
  return {
    ...
    guitar: { body: 'natural', guard: 'none', hardware: 'chrome', inlay: 'dots' }
  };
}
...
export function isOpen(kind, id, progress) {
  const lists = { skin: SKINS, hair: HAIR, hat: HATS, shirt: SHIRTS, body: BODIES, guard: GUARDS, hardware: HARDWARE, inlay: INLAYS };
...
export function lockHint(opt, progress) {
  if (!opt || optionOpen(opt, progress)) return '';
...
export function sanitize(raw, progress) {
  const base = defaults();
  ...
  function pick(list, kind, val, fallback) {
    ...
    if (hit && isOpen(kind, hit.id, progress)) return hit.id;
    return fallback;
  }
  return {
    avatar: {
...
export function loadLocker(progress) {
  if (typeof window === 'undefined') return sanitize(defaults(), progress);
  try {
    const raw = window.localStorage.getItem(KEY);
    return sanitize(raw ? JSON.parse(raw) : defaults(), progress);
  } catch (e) {
    return sanitize(defaults(), progress);
  }
}

export function saveLocker(next, progress) {
  const clean = sanitize(next, progress);
