...
export function enrichLesson(lesson) {
  ...
  var seen = {};
  var steps = [];
  (lesson.steps || []).concat(grouped ? [] : extraSteps(lesson)).forEach(function (s) {
    var k = String(s || '').trim().toLowerCase();
    if (!k || seen[k]) return;
    seen[k] = true;
    steps.push(s);
  });
  var notes = (grouped || change) ? fillBeats(lesson.notes || [], style) : expandNotes(lesson.notes || [], style);
...
export function mergeLessonLists(base, extra) {
  var map = {};
  var order = [];
  (base || []).concat(extra || []).concat(CRAFTED_LESSONS).concat(SKILL_LESSONS).concat(VIB_LESSONS).concat(EXPERT_LESSONS).concat(HAND_DRILLS).concat(CIRCLE_LESSONS).concat(HARMONY_LESSONS).concat(DOMINANT_LESSONS).forEach(function (l) {
    if (!l || !l.id) return;
